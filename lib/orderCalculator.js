...
function calculateOrder({
  ...
  // grossAmount = sum of (ticket price + service fee) per ticket addon (pre-discount)
  ...
  for (const addon of addOns) {
    ...
    // discount (not capped — stored at full promo value)
    let discount = 0;
    if (promotion) {
      const eligibleTypes = promotion["ApplicableAddOns"] || [];
  ...
  // --- Step 2: custom fees ---
  // Percentage fees: applied to gross ticket price per addon (pre-discount, pre-service-fee)
  ...
  for (const customFeeType of Object.values(customFeeTypes)) {
    ...
    if (feeType === "Fixed") {
      ...
      totalCustomFees += roundTo2(totalGrossTicketBase * feeAmt);
    }
  }
  totalCustomFees = roundTo2(totalCustomFees);

  // --- Step 3: processing fees ---
  const providerKey = ((order["OS Payment Provider"] || "").toLowerCase());
